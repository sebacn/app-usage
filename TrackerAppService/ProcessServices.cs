        ...
        */
        ...
        private IntPtr GetEnvironmentBlock(IntPtr token)
        {
            ...
            if (!result)
            {
                ...
                EventLog.WriteEntry("TrackerAppService", $"CreateEnvironmentBlock Error: {Marshal.GetLastWin32Error()}", EventLogEntryType.Error);
            }
        ...
        private IntPtr GetPrimaryToken(Process process)
        {
            ...
            if (openResult)
            {
                ...
                if (!openResult)
                {
                    ...
                    EventLog.WriteEntry("TrackerAppService", $"DuplicateTokenEx Error: {Marshal.GetLastWin32Error()}", EventLogEntryType.Error);
                }
            }
            else
            {
                //_logger?.LogError("OpenProcessToken Error: {0}", Marshal.GetLastWin32Error());
                EventLog.WriteEntry("TrackerAppService", $"OpenProcessToken Error: {Marshal.GetLastWin32Error()}", EventLogEntryType.Error);
            }
            ...
            IntPtr environmentBlock)
        {
            ...
            startupInfo.dwFlags = STARTF_USESTDHANDLES; // STARTF_USESHOWWINDOW | STARTF_FORCEONFEEDBACK | STARTF_USESTDHANDLES;
            ...
            if (!result)
            {
                EventLog.WriteEntry("TrackerAppService", $"CreateProcessAsUser Error: {Marshal.GetLastWin32Error()}", EventLogEntryType.Error);
                //.LogError("CreateProcessAsUser Error: {0}", Marshal.GetLastWin32Error());
            }
            else
            {
